...
export function registerRoutes(api: PluginApi, db: RemiryDb): void {
  ...
  api.registerHttpRoute({ method: "GET", path: "/remiry/items", auth: "gateway", handler: (req) => {
    try {
  ...
  // GET /remiry/items/:id
  api.registerHttpRoute({ method: "GET", path: "/remiry/items/:id", auth: "gateway", handler: (req) => {
    try {
  ...
  // POST /remiry/items
  api.registerHttpRoute({ method: "POST", path: "/remiry/items", auth: "gateway", handler: (req) => {
    try {
  ...
  // PUT /remiry/items/:id
  api.registerHttpRoute({ method: "PUT", path: "/remiry/items/:id", auth: "gateway", handler: (req) => {
    try {
      const id = Number(req.params?.id);
      if (!Number.isInteger(id) || id <= 0) return badRequest("Invalid id");
  ...
  api.registerHttpRoute({ method: "DELETE", path: "/remiry/items", auth: "gateway", handler: (req) => {
    try {
  ...
  api.registerHttpRoute({ method: "DELETE", path: "/remiry/items/:id", auth: "gateway", handler: (req) => {
    try {
      const id = Number(req.params?.id);
      if (!Number.isInteger(id) || id <= 0) return badRequest("Invalid id");
  ...
  api.registerHttpRoute({ method: "GET", path: "/remiry/upcoming/events", auth: "gateway", handler: (req) => {
    try {
  ...
  api.registerHttpRoute({ method: "GET", path: "/remiry/summary", auth: "gateway", handler: (req) => {
    try {
  ...
  api.registerHttpRoute({ method: "GET", path: "/remiry/upcoming/expiry", auth: "gateway", handler: (req) => {
    try {
      const days = Number(req.query?.days ?? "7");
      if (!Number.isInteger(days) || days < 0) return badRequest("days must be a non-negative integer");
