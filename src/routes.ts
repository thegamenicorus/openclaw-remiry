...
export function registerRoutes(api: PluginApi, db: RemiryDb): void {
  ...
  api.registerHttpRoute({ path: "/remiry/items", auth: "gateway", handler: (req) => {
    ...
    try {
      if (method === "GET") {
        ...
        return ok(db.getAll(type).map(serializeItem));
      }
      ...
      if (method === "POST") {
        ...
        return created(serializeItem(db.create(input)));
      }
  ...
  // /remiry/items/:id  — GET (get one), PUT (update), DELETE (delete one)
  api.registerHttpRoute({ path: "/remiry/items/:id", auth: "gateway", handler: (req) => {
    const method = (req.method ?? "GET").toUpperCase();
    try {
  ...
  // /remiry/summary  — GET only
  api.registerHttpRoute({ path: "/remiry/summary", auth: "gateway", handler: (req) => {
    try {
  ...
  // /remiry/upcoming/events  — GET only
  api.registerHttpRoute({ path: "/remiry/upcoming/events", auth: "gateway", handler: (req) => {
    try {
      const days = Number(req.query?.days ?? "7");
      if (!Number.isInteger(days) || days < 0) return badRequest("days must be a non-negative integer");
      return ok(db.upcomingEvents(days).map(serializeItem));
    } catch (err) {
      return serverError(err);
    }
  }});

  // /remiry/upcoming/expiry  — GET only
  api.registerHttpRoute({ path: "/remiry/upcoming/expiry", auth: "gateway", handler: (req) => {
    try {
      const days = Number(req.query?.days ?? "7");
      if (!Number.isInteger(days) || days < 0) return badRequest("days must be a non-negative integer");
      return ok(db.upcomingExpiry(days).map(serializeItem));
    } catch (err) {
      return serverError(err);
    }
  }});
